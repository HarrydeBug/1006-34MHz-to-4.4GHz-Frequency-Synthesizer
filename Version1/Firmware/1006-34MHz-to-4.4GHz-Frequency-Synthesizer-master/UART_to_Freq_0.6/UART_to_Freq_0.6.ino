/* Software for Zachtek "1006-34MHz-to-4.4GHz-Frequency-Synthesizer or Syntehesizer 1" 
   
   Arduino software that takes incoming data on serial port and sets the output frequency on a Synthesizer
   ...
   In and Outputs
    10In  --  PLL Lock detect MUXOUT on ADF4351
    11Out --  SPI Data
    13Out --  SPI Clock
    A0Out --  Turns on or off power to the ADF4351 circuit.
    A1Out --  SPI Latch Enable
    A3Out --  Frequency update indicator LED, flashes when a new command is received on the UART or I2C bus
...
#define ADF4351_LE A1    //Latch enable on pin A1
#define ADF4351_LD 10    //Lock Detect (MUXOUT) on pin  10
#define LDO_Enable A0    //Power regulator Enable on pin A0
#define LEDIndicator1 A3 //LED to indicator for Command received on pin A3
...
uint64_t MinFreq=25000000;
uint64_t ADF4351SetFreq=100000000;
...
void setup() {
  ...
  delay (300);
  Serial.print(F("Zachtek RF Syntesizer 1 Software version: "));
  ...
  pinMode(LEDIndicator1, OUTPUT); // Set Command LED pin as output.
  digitalWrite(LEDIndicator1, HIGH); //Turn on Command LED
  ...
  ADF4351SetFreq=1000000000;
  SetADFFreq ();
  Serial.print ("Frequency set to 1GHz");
...
void loop()
{
    ...
    //Pulse LED if we got a command on the UART or I2C
    if (CommandQuery) {
...
void SCMDHelp () {
  ...
  Serial.println("");
  Serial.println(F(" Commands are case sensitive, use only upper case"));
  Serial.println("");
  CommandQuery=true;
}
